...
def go(args):
    ...
    X_val_imputed = sk_pipe.named_steps['preprocessor'].transform(X_val)
    ...
    signature = infer_signature(X_val_imputed, y_pred)
    mlflow.sklearn.save_model(
        sk_model=sk_pipe,
        path=rf_storage_dir,
        serialization_format=mlflow.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE,
        signature=signature,
        input_example=X_val_imputed[:15]
    )
    ...
    artifact.add_dir(os.path.join(rf_storage_dir,))
